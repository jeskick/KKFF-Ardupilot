...
| 基线版本 | **4.5.7**（稳定）、**4.7.0-beta7**（beta，当前主线） |
...
ardupilot.4.7.0beta7_FF_RADAR
    = base/ardupilot.4.7.0beta7 + feature/FF_RADAR（4.7 移植）+ H7A3 板级适配

ardupilot.4.7.0beta7_FF_RADAR_TKOFF
    = ardupilot.4.7.0beta7_FF_RADAR + feature/FF_TKOFF（4.7 移植）
```
...
| `ardupilot.<版本>_FF_<功能>` | 产品分支 | `ardupilot.4.7.0beta7_FF_RADAR` |
...
| 目录 | 分支 | 用途 |
|------|------|------|
| `FFardupilot` | `ardupilot.4.7.0beta7_FF_RADAR` | 主编译（H7A3 等 4.7 板） |
...
### 功能 3：Matek H7A3 板载 INA2xx 电流计（仅 4.7 产品分支）
...
**所在分支：** `ardupilot.4.7.0beta7_FF_RADAR`（及 `ardupilot.4.7.0beta7_FF_RADAR_TKOFF`）。
...
| 我要… | 产品分支 | 典型板型 |
|--------|----------|----------|
| 4.7 + 编队 + H7A3 电流计 | `ardupilot.4.7.0beta7_FF_RADAR` | `MatekH7A3-Wing` |
| 4.5.7 + 编队 | `ardupilot.4.5.7_FF_RADAR` | `MatekF405-Wing` |
| 4.5.7 + 编队 + TKOFF | `ardupilot.4.5.7_FF_RADAR_TKOFF` | `MatekF405-Wing` |
| 4.7 + 编队 + TKOFF | `ardupilot.4.7.0beta7_FF_RADAR_TKOFF` | `MatekH7A3-Wing` |
...
### 4.7.0-beta7 产品 `ardupilot.4.7.0beta7_FF_RADAR` 相对 `base/ardupilot.4.7.0beta7`

| 提交 | 内容 |
|------|------|
| formation port | 编队雷达完整移植到 4.7.0-beta7（AP_Radar、MSP、OSD、Plane 集成） |
...
| 改编队+刷机 | 直接在 `ardupilot.4.7.0beta7_FF_RADAR` 改并提交 |
...
/usr/bin/git checkout ardupilot.4.7.0beta7_FF_RADAR
...
```bash
/usr/bin/git push jeskick ardupilot.4.7.0beta7_FF_RADAR
```
...
```bash
/usr/bin/git fetch upstream
/usr/bin/git log --oneline ardupilot.4.7.0beta7_FF_RADAR..upstream/master   # 官方有、你还没有的
/usr/bin/git log --oneline upstream/master..ardupilot.4.7.0beta7_FF_RADAR   # 你自己的定制提交
```
...
```bash
git push -u jeskick ardupilot.4.7.0beta7_FF_RADAR
```
